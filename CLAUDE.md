...
**Capa estética:** el bloque `/* ===== Capa estética ===== */` del CSS (justo antes de «Responsive») redefine tokens (`--bg`, `--borde`, `--campo` para el fondo de los campos, `--sombra`, `--radio`) y afina tarjetas, indicadores, formularios, tablas, modal e ingreso. Va **antes** de «Responsive» a propósito para que los ajustes de celular sigan mandando: los estilos nuevos de celular van en ese bloque, no después. En oscuro el menú lateral deja el rosa pleno y pasa a ciruela con el rosa de acento. Los íconos del menú son SVG de línea (`ICONOS_NAV`, clave = id de la pantalla); una pantalla nueva sin ícono en ese mapa usa su emoji de `TABS`. El banner de Inicio usa la clase `.hero`.
...
| `movimientos/<suc>/<YYYY-MM-DD>/<id>` | Una fila de la planilla: `hora, clienteId, clienteNombre, servicioId, servicioNombre, tipo, equipoId, sesion (bool: consume ficha), forma ('efectivo'|'tarjeta'|'transferencia'|'abono'), importe, abonoId (sesión que descuenta de un abono), abonoCreadoId (venta de abono), nota, usuario, responsableId, responsable` (responsable = quién hizo la carga, elegido entre los «a cargo» del día; obligatorio si hay alguien a cargo). Gastos y retiros también guardan `responsable`. |
| `historial/<clienteId>/<movId>` | Copia liviana del movimiento, para la ficha del cliente (se escribe en el mismo `update`). |
| `cajas/<suc>/<YYYY-MM-DD>` | `inicial, gastos/<id>{desc,importe}, retiros/<id>{quien→desc,importe}, observaciones, fichas/{gris,dorada}/{inicio,repuestas}, control/<equipoId>/sacadas, aCargo/<personalId>={nombre, turno, reemplazo, desde} (días viejos: sólo el nombre como texto; leer siempre con `datoACargo()`), supervisor{id,nombre}, resumen{…}`. Se guarda el **nombre** además del id para que el historial se lea aunque la ficha cambie o se borre. |
| `roles/<uid>` | `rol ('admin'|'recepcion'|'lector'), sucursal, email` |
| `solicitudes/<uid>` | Cuentas que ingresaron y todavía no tienen rol. |
| `config/personal/<id>` | `nombre, puesto ('encargada'|'recepcionista'|'supervisora'), sucursalId ('' = todas), turno ('manana'|'tarde'|'noche'|'completo'|'rotativo'), horario (texto), reemplazos (bool: aparece como «🔁 Reemplazo» en la planilla de otras sucursales), telefono, dni, notas, activo, orden`. Vive dentro de `config` a propósito: así lo cubre la regla de admin ya publicada y no hizo falta tocar las reglas (`RUTA_COL` mapea la colección `personal` a esa ruta para ordenar). |
